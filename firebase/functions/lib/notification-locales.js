...
exports.getSmartReminderCopy = getSmartReminderCopy;
exports.getScheduledReminderCopy = getScheduledReminderCopy;
const ENGLISH_SMART = {
    "low-storage": {
...
function normalizedLocale(locale) {
    ...
    if (language === "zh")
        return value.toLowerCase().includes("hant") ? "zh-Hant" : "zh-Hans";
    if (language === "pt")
        return "pt-BR";
    return SMART[language] ? language : "en";
}
function getSmartReminderCopy(locale, trigger) {
    return (SMART[normalizedLocale(locale)] ?? ENGLISH_SMART)[trigger] ?? ENGLISH_SMART[trigger];
}
function getScheduledReminderCopy(locale, label, targetMB) {
    ...
    const safeTarget = Number.isFinite(targetMB) ? Math.max(10, Math.round(targetMB)) : 50;
    const templates = {
...
//# sourceMappingURL=notification-locales.js.map