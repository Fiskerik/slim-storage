...
const fields = ["subtitle", "promotionalText", "description", "keywords", "reviewNotes", "releaseNotes"];
const chars = (value, max) => [...value].slice(0, max).join("");
const bytes = (value, max) => { let result = ""; for (const char of value) { if (Buffer.byteLength(result + char, "utf8") > max) break; result += char; } return result; };
...
const listings = await Promise.all(metadata.locales.map(async (locale) => {
  ...
  const translated = Object.fromEntries(await Promise.all(fields.map(async (field) => [field, await translate(metadata.template[field], target)])));
  return { locale: locale.id, language: localeNames[locale.id] ?? locale.language, name: metadata.template.name, subtitle: chars(translated.subtitle, 30), promotionalText: chars(translated.promotionalText, 170), description: chars(translated.description, 4000), keywords: bytes(translated.keywords, 100), reviewNotes: translated.reviewNotes, releaseNotes: translated.releaseNotes, screenshots: metadata.sharedScreenshotPaths };
}));
