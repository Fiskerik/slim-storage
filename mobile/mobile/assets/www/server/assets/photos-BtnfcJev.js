...
const PIC = (seed, w = 800, h = 1100) => `https://picsum.photos/seed/${seed}/${w}/${h}`;
const SAMPLE_PHOTOS = [
  ...
  { id: "b4b", url: PIC("latte-burst-2"), thumb: PIC("latte-burst-2", 400, 500), title: "Latte Art", year: 2024, month: "Jan", device: "iPhone 15", sizeMB: 3, hasGPS: false, burstId: "burst-latte", burstOffset: 1 }
];
Object.values(
  SAMPLE_PHOTOS.reduce((acc, p) => {
    if (!p.burstId) return acc;
    (acc[p.burstId] ||= []).push(p);
    return acc;
  }, {})
).filter((g) => g.length >= 2);
const MEMORY_POOL = SAMPLE_PHOTOS.filter((p) => p.year <= 2020 && !p.burstId);
export {
